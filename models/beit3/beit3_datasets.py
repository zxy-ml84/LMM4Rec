...
from models.beit3.glossary import normalize_word
from models.beit3.randaug import RandomAugment

# AtoMiC
from datasets import load_dataset
from models.beit3.utils import _log_time_usage
...
class AtoMicDataset(torch.utils.data.Dataset):
    def __init__(self, args, split, transform,
                 ...
                 data_path=None, load_text_on_fly=True):

        # split = 'validation'
        # load dataset from huggingface
        with _log_time_usage("load images and qrels data from huggingface hub: "):
            print("load images and qrels data from huggingface hub: ")
        ...
        self.load_text_on_fly = load_text_on_fly

        # load tokenised text data from json file
        # load tokenised text data from json file
        if not load_text_on_fly:
        ...
        text_ids = [text_id['text_id'] for text_id in self.texts]
        self.text_id2row_dict = self._getRowIdx(text_ids)

        # remove images do not need in this split
        # print("remove images do not need in this split: ")
        # self.images = self._split_images(images, self.qrels)

        # build index to map image_id to index in self.images
        ...
        if text_features is None:
            text_features = ["context_page_des_tokens_ids", "context_sec_des_tokens_ids",
                             "page_title_tokens_ids", "section_title_tokens_ids", ]
    ...
    def _tokenize_text(self, new_dict):
        result = {}
        context_page_des_tokens = self.tokenizer.tokenize(new_dict["context_page_description"])
        context_sec_des_tokens = self.tokenizer.tokenize(new_dict["context_section_description"])
        page_title_tokens = self.tokenizer.tokenize(new_dict["page_title"])
        section_title_tokens = self.tokenizer.tokenize(new_dict["section_title"])
        # category_tokens = tokenizer.tokenize(''.join(new_dict["category"]))

        context_page_des_tokens_ids = self.tokenizer.convert_tokens_to_ids(context_page_des_tokens)
        context_sec_des_tokens_ids = self.tokenizer.convert_tokens_to_ids(context_sec_des_tokens)
        page_title_tokens_ids = self.tokenizer.convert_tokens_to_ids(page_title_tokens)
        section_title_tokens_ids = self.tokenizer.convert_tokens_to_ids(section_title_tokens)
        ...
        result["context_page_des_tokens_ids"] = context_page_des_tokens_ids
        result["context_sec_des_tokens_ids"] = context_sec_des_tokens_ids
        result["page_title_tokens_ids"] = page_title_tokens_ids
        result["section_title_tokens_ids"] = section_title_tokens_ids
...
class RecsysDataset(Dataset):
    def __init__(self, args, ann_file, transform,
                 num_max_bpe_tokens, tokenizer,):

        # items = []
        # with open(ann_file, mode="r", encoding="utf-8") as reader:
        #     for line in reader:
        #         data = json.loads(line)
        #         items.append(data)
        ...
        self.item_num = len(self.ann)
        self.transform = transform
        self.num_max_bpe_tokens = num_max_bpe_tokens

        self.image_root = args.image_root # args.data_path + '/raw_image_id'
    ...
    def _get_image(self, image_path: str):
        ...
        img_path = os.path.join(self.image_root, image_path + '.jpg')
    ...
    def _tokenize_text(self, text_segment):
        tokens = self.tokenizer.tokenize(text_segment[:self.args.num_max_bpe_tokens])
    ...
    def _get_image_text_example(self, index: int, data: dict):
        ...
        data['image_id'] = int(img_path)
        img = self._get_image(img_path)
        data["image"] = img
        # data["image_id"] = item["image_id"]

        text_segment = item["text_segment"]
        text_segment = self._tokenize_text(text_segment)
        language_tokens, padding_mask, _ = self._get_text_segment(text_segment)
        data["language_tokens"] = language_tokens
        data["padding_mask"] = padding_mask
...
def create_dataset_by_split_recsys_dataset(args, split, is_train=True):
    transform = build_transform(is_train=is_train, args=args)
    # dataset_class = movie_dataset[args.task]
    tokenizer = get_sentencepiece_model_for_beit3(args)
    ...
    dataset = RecsysDataset(args,
        ann_file= args.json_path,
        transform=transform, tokenizer=tokenizer,
        num_max_bpe_tokens=args.num_max_bpe_tokens,
    )
    ...
    if is_eval:
        ...
        return \
            create_dataset_by_split(args, split="train", is_train=True), \
                create_dataset_by_split(args, split="validation", is_train=True)

def create_recsys_dataset(args, split="all", is_train=True):
    return create_dataset_by_split_recsys_dataset(args, split=split, is_train=is_train)