...
def compute_irtr(pl_module, batch, aggregate=False):
    ...
    if pl_module.distill:
        with torch.no_grad():
            ...
            ground_truth = torch.arange(len(logits_per_image_m)).long().to(device=logits_per_image.get_device())

            irtr_loss_m = (
                                F.cross_entropy(logits_per_image_m.float(), ground_truth)
                                + F.cross_entropy(logits_per_text_m.float(), ground_truth)
                        ) / 2

            irtr_loss = pl_module.alpha * irtr_loss_m + (1 - pl_module.alpha) * irtr_loss

            pl_module._dequeue_and_enqueue(image_features_m, text_features_m, None, None)
...
def compute_irtr_recall(pl_module, split="test"):
    ...
    text_loader = torch.utils.data.DataLoader(
        text_dset,
        batch_size=32,
        num_workers=2,  # pl_module.config["num_workers"],
        pin_memory=True,
...
def compute_irtr_recall_with_rerank(pl_module, split="test"):
    ...
    k_test = pl_module.config["k_test"]
    ...
    for i, sims in enumerate(scores[start:end]):
        ...
        cur_rerank_batch = {"image": [cur_images], "text_ids": cur_text_ids, "text_labels": cur_text_labels,
